#include<iostream>
//#define EIGEN_USE_MKL_ALL
#include"basis.hpp"
#include"operators.hpp"
#include"diag.h"
#include"tpoperators.hpp"
#include"files.hpp"
#include"timeev.hpp"
#include"ETH.hpp"
#include<iomanip>
#include <boost/program_options.hpp>

using namespace boost::program_options;
int main(int argc, char *argv[])
{
  using namespace Eigen;
using namespace std;
using namespace Many_Body;
  using Fermi_HubbardBasis= TensorProduct<ElectronBasis, ElectronBasis>;
   using HolsteinBasis= TensorProduct<ElectronBasis, PhononBasis>;
  using Mat= Operators::Mat;
   using boost::program_options::value;
   int N{};
  int L{};
  int i{};
  int j{};
    double T{};
      double tot{};
        double dt{};
  double t0{};
  double omega{};
  double gamma{};
  double cutoff{};
  bool PB{};


  std::string sN{};
  std::string sL{};
  std::string si{};
  std::string sj{};
  std::string star{};
  std::string sT{};
    std::string stot{};
      std::string sdt{};
  std::string st0{};
  std::string somega{};
  std::string sgamma{};
  std::string starget{};
  std::string scutoff{};
  std::string sPB{};
  std::string filename="freefermSPexT";
  try
  {
    boost::program_options::options_description desc{"Options"};
    desc.add_options()
      ("help,h", "Help screen")
      ("L", value(&L)->default_value(4), "L")
      ("i", value(&i)->default_value(0), "i")
      ("j", value(&j)->default_value(1), "j")
      ("T", value(&T)->default_value(0.1), "T")
      ("N,n", value(&N)->default_value(2), "N")
      ("tot", value(&tot)->default_value(1.), "tot")
      ("dt", value(&dt)->default_value(0.1), "dt")
      ("t0", value(&t0)->default_value(1.), "t0")
      ("gam", value(&gamma)->default_value(1.), "gamma")
      ("omg", value(&omega)->default_value(1.), "omega")
      ("pb", value(&PB)->default_value(0), "pb");
  boost::program_options::variables_map vm;
  boost::program_options::store(parse_command_line(argc, argv, desc), vm);
  boost::program_options::notify(vm);

  if (vm.count("help"))
      {std::cout << desc << '\n'; return 0;}
    else{
      if (vm.count("L"))
      {      std::cout << "L: " << vm["L"].as<int>() << '\n';
      	sL="L"+std::to_string(vm["L"].as<int>());
	filename+=sL;
      }
         if (vm.count("N"))
      {      std::cout << "N: " << vm["N"].as<int>() << '\n';
      	sN="N"+std::to_string(vm["N"].as<int>());
	filename+=sN;
      }
	   if (vm.count("i"))
      {      std::cout << "i: " << i << '\n';
      	si="i"+std::to_string(i);
	filename+=si;
      }
	   if (vm.count("j"))
      {      std::cout << "j: " << j << '\n';
      	sj="j"+std::to_string(j);
	filename+=sj;
      }
      	 if (vm.count("t0"))
      {      std::cout << "t0: " << vm["t0"].as<double>() << '\n';
      	st0="t0"+std::to_string(vm["t0"].as<double>()).substr(0, 3);
      	filename+=st0;
      }

		      		 if (vm.count("tot"))
      {      std::cout << "tot: " << vm["tot"].as<double>() << '\n';
      	stot="tot"+std::to_string(vm["tot"].as<double>()).substr(0, 3);
      		filename+=stot;
      }
				 if (vm.count("dt"))
      {      std::cout << "dt: " << vm["dt"].as<double>() << '\n';
      	sdt="dt"+std::to_string(vm["dt"].as<double>()).substr(0, 4);
      		filename+=sdt;
      }

      		 if (vm.count("T"))
      {      std::cout << "T: " << T << '\n';
      	sT="T"+std::to_string(vm["T"].as<double>()).substr(0, 3);
      	filename+=sT;
      }
		 if (vm.count("pb"))
      {      std::cout << "PB: " << vm["pb"].as<bool>() << '\n';
      	sPB="PB"+std::to_string(vm["pb"].as<bool>());
      		filename+=sPB;
      }
      }
    }
  catch (const boost::program_options::error &ex)
  {
    std::cerr << ex.what() << '\n';
    return 0;
  }
  filename+=".bin";
  

  // declaring the tensor products with 0 e, 1, e, 2 e
  ElectronBasis e( L);
  //std::cout<< e.dim<< std::endl;
   //std::cout<< e << std::endl;
    auto Ham=Operators::EKinOperator(e, t0, PB);


        Eigen::VectorXd eigenVals(e.dim);

      Eigen::MatrixXd HH=Eigen::MatrixXd(Ham);

  	  Many_Body::diagMat(HH, eigenVals);

	  
   std::cout<< eigenVals(0)<< std::endl;  
   //std::cout<< Eigen::MatrixXd(C)<< std::endl;
       Eigen::VectorXd inS=HH.col(0); 
       std::cout<<"MIN E0 "<<std::setprecision(15)<< eigenVals(0)<< std::endl;

       for(int b=0; b<L; b++)
	 {
	   auto N_e=Operators::NumberOperatorE( e, 1., PB, b, b+1);
	   //	   std::cout<<N_e << std::endl;

       std::cout<< " Exp "<< b << "  "<<inS.transpose()*N_e*inS<< std::endl;
}
       //       Eigen::VectorXd inS=HH.col(0);
       
       // inS(0)=1;
  auto N_e=Operators::NumberOperatorE( e, 1., PB);
       std::cout<< " tot Exp "<<inS.transpose()*N_e*inS<< std::endl;
         int n=0;
        Eigen::MatrixXcd evExpbeta=Eigen::MatrixXcd::Zero(eigenVals.rows(), eigenVals.rows());
         double Z{0};
	 //	 std::cout<< " Exp "<< inS.transpose()*N_e*inS<<std::endl;
// 	auto it=TP.basis.begin();
	 std::vector<double > ev;
    
      std::vector<double> time;
      std::vector<std::complex<double>> cdagc;
      std::vector<std::complex<double>> ccdag;
      auto HT=HH.transpose();

//        while(n*dt<tot)
//        	{
     	  	  

//          Eigen::MatrixXcd evExpm=TimeEv::EigenvalExponent(eigenVals, n*dt);
//             Eigen::MatrixXcd evExpp=TimeEv::EigenvalExponent(eigenVals, -n*dt);

//  	    auto CDAGC=(inS.transpose())*Cdag*HH*evExpm*HT*C*inS;
// 	     auto CCDAG= (inS.transpose())*C*HH*evExpp*HT*Cdag*in




      int ii=int(L/2);
      std::cout<< " i " <<ii<< std::endl;
      double s1{0};
      double s2{0};
      for(int jj=0; jj<L; jj++)
	{
	  auto Cdag=Operators::CdagOperator(e, ii,  PB);
	   auto C=Operators::COperator(e, jj,  PB);
auto CDAGC=(inS.transpose())*Cdag*C*inS;
 	     auto CCDAG= (inS.transpose())*C*Cdag*inS;
	     s1+=CDAGC;
	     s2+=CCDAG;
//     using Many_Body::im;
     std::cout<<jj << "  CDAGC \t"<< CDAGC<< " and CCDAG "<<CCDAG<< "sum "<< CDAGC+CCDAG<<std::endl;
	}
      std::cout<< s1<< " and " << s2<<std::endl;






// /// 		      std::cout<<n*dt << "\t"<<  " and  "<<CCDAG<< "  "<< "  sum  " <<Z<<std::endl;
//        			    	    time.push_back(n*dt);
//      			    	    ccdag.push_back(CCDAG);
//      			    cdagc.push_back(CDAGC)
// 							  ;
//    n++;

//    	 	}
//   // 	std::cout<< ccdag.size() << "  "<< cdagc.size()<< std::endl;
//        bin_write("time"+filename, time);
//        bin_write("CCDAG"+filename, ccdag);
//        bin_write("CDAGC"+filename, cdagc);
  
 
       // 			  int pb=PB;
       // 		  	  std::string Hs="H";
       // 			  std::string Ts="T";
       // 			  std::string phds="PHD";
	  
       // 		      bin_write("E"+filename, Evec);
       // 		      bin_write("Nph"+filename, Ovec);
       // 		      bin_write("EK"+filename, Ovec2);
       // 		      bin_write("nX"+filename, Ovec3);
       // 		      bin_write("temp"+filename, Tr);
  return 0;
}
